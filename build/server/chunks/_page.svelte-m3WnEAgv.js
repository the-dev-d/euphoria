...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  do {
    ...
    $$rendered = `<main class="w-full h-screen flex items-center justify-center"><div class="w-[800px] h-[auto] flex bg-[#1E1E1E] rounded-md overflow-hidden shadow-md border border-[#3BE4C4] m-3"><div class="w-full md:w-1/2 p-8 flex flex-col justify-center"><form method="POST" class="w-full space-y-4"> <h1 class="text-3xl text-center font-anton mb-8 text-white" data-svelte-h="svelte-19xcgaf">REGISTRATION</h1> <div class="flex flex-col gap-2">${validate_component(Label, "Label").$$render($$result, { for: "name" }, {}, {
      ...
      {}
    )} ${name && nameError ? `<p class="text-red-400 text-xs">${escape(nameError)}</p>` : ``}</div> <div class="flex flex-col gap-2">${validate_component(Label, "Label").$$render($$result, { for: "email" }, {}, {
      default: () => {
        return `Email`;
      }
    })} ${validate_component(Input, "Input").$$render(
      ...
      {}
    )} ${confirmPassword && confirmPassword !== password ? `<p class="text-red-400 text-xs" data-svelte-h="svelte-89de70">Password does not match</p>` : ``}</div> <p class="text-xs my-2" data-svelte-h="svelte-ecfv2z">Please bring a Valid ID proof for verification.</p> <div class="flex flex-col gap-2">${validate_component(Button, "Button").$$render(
      $$result,
      {
        class: " bg-[#3BE4C4]",
    ...
    )} <p class="text-sm flex gap-3 mt-2" data-svelte-h="svelte-nitxf1"><span>Already Registered?</span> <a href="/respawn/streamersbattle/login" class="text-[#3BE4C4]">Login</a></p></div> ${form?.success ? `<p class="text-sm text-green-500" data-svelte-h="svelte-z2yv6r">Registration successfull</p>` : `${form && !form.success && form.message ? `<p class="text-sm text-red-400 my-2">${escape(form.message)}</p>` : ``}`}</form></div> <div class="w-1/2 hidden bg-[#0F2027] md:flex flex-col items-center justify-center bg-cover bg-center" style="background-image: url('/images/forms/fggaming.jpg');" data-svelte-h="svelte-wi2k4p"><img src="/images/forms/Logoc.png" alt="Logo" class="image-logo spacing w-80 h-auto svelte-9gngtt" style="max-width: 100%; height: auto;"> <img src="/images/forms/Typeface.png" alt="Typeface" class="image-typeface w50 h-auto" style="max-width: 100%; height: auto;"></div></div> </main>`;
  } while (!$$settled);
  return $$rendered;
});

export { Page as default };
//# sourceMappingURL=_page.svelte-m3WnEAgv.js.map
